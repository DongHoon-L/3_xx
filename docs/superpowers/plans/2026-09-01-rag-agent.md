...
def test_answered_query_event(recorder, tmp_path):
    ...
    assert mask_text((tmp_path / "chain.jsonl").read_text(encoding="utf-8"))[1] == []
...
Run: `PY -m pytest`
Expected: audit-engine 85 + rag-agent 99 = 184 passed (`test_api.py` 12 passed 포함)
...
- `PY -m pytest` 전체 통과 (audit-engine 85 + rag-agent 99 = 184).
